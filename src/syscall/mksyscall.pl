...
while(<>) {
	...
	if (($darwin || $openbsd) && $func eq "ptrace") {
	...
	if($darwin) {
		if (not exists $trampolines{$funcname}) {
			...
			# (The jump to this function is in the assembly trampoline, generated by mkasm_darwin.go.)
	...
	if($openbsd) {
		if (not exists $trampolines{$funcname}) {
			...
			$text .= "func ${funcname}_trampoline()\n";
			# Map syscall.funcname to just plain funcname.
			# (The jump to this function is in the assembly trampoline, generated by mkasm_openbsd.go.)
			$text .= "//go:linkname $funcname $funcname\n";
			# Tell the linker that funcname can be found in libc.so using varname without the libc_ prefix.
			my $basename = substr $funcname, 5;
			$text .= "//go:cgo_import_dynamic $funcname $basename \"libc.so.96.0\"\n\n";
		}
	}
}
